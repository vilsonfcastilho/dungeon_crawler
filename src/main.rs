...
impl State {
    fn new() -> Self {
        ...
        let mut map_builder: MapBuilder = MapBuilder::new(&mut rng);

        spawn_player(&mut ecs, map_builder.player_start);

        let exit_idx: usize = map_builder.map.point2d_to_index(map_builder.amulet_start);
        map_builder.map.tiles[exit_idx] = TileType::Exit;

        map_builder
            .monster_spawns
            .iter()
            .for_each(|pos: &Point| spawn_entity(&mut ecs, &mut rng, *pos));
    ...
    fn reset_game_state(&mut self) {
        ...
        map_builder
            .monster_spawns
            .iter()
            .for_each(|pos: &Point| spawn_entity(&mut self.ecs, &mut rng, *pos));
    ...
    fn advance_level(&mut self) {
        let player_entity: Entity = *<Entity>::query()
        ...
        let mut cb: CommandBuffer = CommandBuffer::new(&mut self.ecs);
