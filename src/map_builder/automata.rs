...
impl MapArchitect for CellularAutomataArchitect {
    fn new(&mut self, rng: &mut RandomNumberGenerator) -> MapBuilder {
        let mut mb: MapBuilder = MapBuilder {
            map: Map::new(),
            rooms: Vec::new(),
        ...
        let start: Point = self.find_start(&mut mb.map);
...
impl CellularAutomataArchitect {
    fn random_noise_map(&mut self, rng: &mut RandomNumberGenerator, map: &mut Map) {
        map.tiles.iter_mut().for_each(|t: &mut TileType| {
            let roll: i32 = rng.range(0, 100);
    ...
    fn iteration(&mut self, map: &mut Map) {
        let mut new_tiles: Vec<TileType> = map.tiles.clone();

        for y in 1..SCREEN_HEIGHT - 1 {
            for x in 1..SCREEN_WIDTH - 1 {
                let neighbors: usize = self.count_neighbors(x, y, map);
    ...
    fn find_start(&self, map: &Map) -> Point {
        let center: Point = Point::new(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2);

        let closest_point: usize = map
            .tiles
            .iter()
            .enumerate()
