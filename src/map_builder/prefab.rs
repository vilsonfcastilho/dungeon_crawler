...
pub fn apply_prefab(mb: &mut MapBuilder, rng: &mut RandomNumberGenerator) {
    ...
    let dijkstra_map: DijkstraMap = DijkstraMap::new(
        SCREEN_WIDTH,
        SCREEN_HEIGHT,
    ...
    while placement.is_none() && attempts < 10 {
        let dimensions: Rect = Rect::with_size(
        ...
        dimensions.for_each(|pt| {
            let idx: usize = mb.map.point2d_to_index(pt);
            let distance: f32 = dijkstra_map.map[idx];
        ...
        if can_place {
            ...
            let points: std::collections::HashSet<Point> = dimensions.point_set();
            mb.monster_spawns.retain(|pt: &Point| !points.contains(pt));
        }
